...
@Component({
  selector: 'app-root',
  imports: [GoogleMap, MapAdvancedMarker, MapPolyline, ListUser],
...
export class App implements OnInit {
  ...
  getPolylineSegments() {
    ...
    if (ms.length < 2) return { first: [], second: [] };
    const start = ms[0];
    const end = ms[1];
    // Calcular punto medio geodésico
    const latMid = (start.lat + end.lat) / 2;
    const lngMid = (start.lng + end.lng) / 2;
    const mid = { lat: latMid, lng: lngMid };
    return {
      first: [start, mid],
      second: [mid, end],
    };
